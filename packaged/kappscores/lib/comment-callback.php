<?php
function kappscores_comment_callback( $comment, $args, $depth) {
    ...
    <<?php echo $tag ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( ['c-comment', $comment->comment_parent ? 'c-comment--child' : ''] ) ?>>
        <article id="div-comment-<?php comment_ID(); ?>" class="c-comment__body">
            ...
            <?php edit_comment_link( esc_html__( 'Edit Comment', 'kappscores' ), '<span class="c-comment__edit-link">', '</span>' ); ?>
            ...
            <div class="c-comment__content">
                ...
                <a class="c-comment__time" href="<?php echo esc_url( get_comment_link( $comment, $args ) ) ?>">
                    <time datetime="<?php comment_time( 'c' ) ?>">
                        <?php
                            printf( esc_html__( '%s ago', 'kappscores' ), human_time_diff( get_comment_time( 'U' ), current_time( 'U' ) ));
                        ?>
                    </time>
                </a>

                <?php if($comment->comment_approved == '0') { ?>
                    <p class="c-comment__awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'kappscores' ); ?></p>
                <?php } ?>
